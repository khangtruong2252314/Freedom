...
class Emitter():
    ...
    def emitPUSHICONST(self, in_, frame):
        ...
        frame.push()
        if type(in_) is int or type(in_) is float:
            i = int(in_)
            if i >= -1 and i <= 5:
    ...
    *    @param typ the type of the constant
    ...
    def emitPUSHCONST(self, in_, typ, frame):
        ...
        if type(typ) is StringType:
            frame.push()
            return self.jvm.emitLDC("\"" + in_ + "\"")
        elif type(typ) is NumberType:
            frame.push()
            return self.jvm.emitLDC(in_)
        elif type(typ) is BoolType:
            frame.push()
            return self.jvm.emitICONST(0 if in_ != 'true' else 1)
        else:
    ...
    def emitALOAD(self, in_, frame):
        ...
        frame.pop()
        if type(in_) is cgen.NumberType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        frame.push()
        if type(inType) is cgen.BoolType:
            return self.jvm.emitILOAD(index)
        elif type(inType) is cgen.NumberType:
            return self.jvm.emitFLOAD(index)
        # elif type(inType) is cgen.ArrayPointerType or type(inType) is cgen.ClassType or type(inType) is StringType:
        elif type(inType) is cgen.StringType or type(inType) is cgen.ArrayType:
    ...
    *   @param name the symbol entry of the variable.
    ...
    def emitWRITEVAR(self, name, inType, index, frame):
        ...
        # elif type(inType) is cgen.ArrayPointerType or type(inType) is cgen.ClassType or type(inType) is StringType:
        elif type(inType) is cgen.StringType or cgen.ArrayType:
    ...
    *   @param in the type of the operands.
    ...
    def emitADDOP(self, lexeme, in_, frame):
        ...
        def true_op():
            obj = self.jvm
            return obj.emitFADD() if lexeme == '+' else obj.emitFSUB()
    ...
    *   generate ior
    ...
    def emitFREOP(self, op, frame):
        ...
        if op == ">":
            ...
            raise RuntimeError('What the hell is == in this ops')
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) in [cgen.StringType, cgen.ArrayType]:
